...
<div class="md:px-20 py-4 pt-8 md:py-8 px-4">
  <div class="inline-flex items-center w-full md:px-4">
  ...
  <button class="md:hidden inline-flex items-center py-2">
    <span class="font-medium">Filter</span>
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
...
<div class="px-4 md:px-24">
  {#if isLoading}
    ...
    {#each [1, 2, 3, 4] as _ (_)}
      {#if contentWidth < 600}
      {@const newContentWidth = contentWidth + 0.05 * contentWidth}
      <Skeleton height={350} width="{contentWidth + 0.05 * newContentWidth}">
        <rect width="{newContentWidth}" height="300" x="0" y="0" rx="12" ry="12" />
        <rect width="{newContentWidth}" height="10" x="0" y="315" rx="5" ry="5" />
        <rect width="{newContentWidth/1.45}" height="10" x="0" y="335" rx="5" ry="5" />
      </Skeleton>
      {:else}
      <Skeleton height={410} width="{contentWidth}">
        ...
        <rect width="{contentWidth/1.45}" height="15" x="0" y="390" rx="5" ry="5" />
      </Skeleton>
      {/if}
    {/each}
    </div>
    ...
    <div transition:fade="{{delay: 200, duration: 200}}" class="grid grid-cols-1 w-full gap-10 md:gap-14 mb-12 md:grid-cols-2">
